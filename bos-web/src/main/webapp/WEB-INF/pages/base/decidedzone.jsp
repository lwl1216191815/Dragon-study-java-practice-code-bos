...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(rows.length!=1){
			...
			var url_left="decidedzoneAction_findCustomerListNotAssociation.action";
			$.post(url_left,function(data){
			...
			var url_right="decidedzoneAction_findCustomerListHasAssociation.action";
			var decidedzoneId = rows[0].id;
			$.post(url_right,{"id":decidedzoneId},function(data){
				for(var i = 0; i<data.length;i++){
					var id =data[i].id;
					var name = data[i].name;
					var telephone =data[i].telephone;
	...
	function doDblClickRow(index,data){
		$('#association_subarea').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "subareaAction_findSubareaListByDecidedzoneId.action?decidedzoneId="+data.id,
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "decidedzoneAction_findCustomerListHasAssociation.action?id="+data.id,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
			...
			<form id="addDecidedzoneForm" method="post" action="decidedzoneAction_addDecidedzone.action">
				<table class="table-edit" style="width:80% ; align:center">
					...
					<tr>
						<td>选择取派员</td>
						<td>
							<input class="easyui-combobox" name="staff.id" data-options="valueField:'id',textField:'name',url:'staffAction_listajax.action'" />  
						</td>
					</tr>
					<tr height="300">
	...
	<div class="easyui-window" title="查询定区窗口" id="searchWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div style="overflow:auto;padding:5px;" border="false">
			<form>
				<table id="selectDecidedzoneTable" class="table-edit">
					<tr class="title">
